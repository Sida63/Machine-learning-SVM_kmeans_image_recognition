...
samplenumber=100
tempx=[2,2]
for k_num in range(1,31):
    SvmList=[]
    print("FirstK:"+str(k_num))
        ...
        for sampleindex2 in range(sampleindex1+1,5):
            ...
            for i in xrange(samplenumber):
                try:
                    ...
                    sift = cv2.SIFT()
                    kp, des = sift.detectAndCompute(gray,None)
                    temptest, classified_pointstest, meanstest=cv2.kmeans(data=des,K=k_num,bestLabels=None,criteria=(cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_MAX_ITER, 1000, 0.001),attempts=10,flags=cv2.KMEANS_RANDOM_CENTERS)                    meanstest=meanstest.reshape(-1)
                    training_set.append(meanstest)
